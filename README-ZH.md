...
MCP Server 只监听 `127.0.0.1`。保持一个锐捷项目标签页打开即可，无需一直停留在该标签页。Agent 可使用十个只读工具，覆盖项目/设备状态、告警、拓扑、客户端、操作日志、无线设置和 Portal 认证。客户端列表采用精简分页结果，支持直属设备或拓扑子树范围，完整记录只通过 `get_client_info` 返回；设备 SN 必须来自当前项目，不能调用任意 API 或写操作。
