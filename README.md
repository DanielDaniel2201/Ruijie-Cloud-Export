...
The server binds only to `127.0.0.1`. Keep one Ruijie project tab active. The agent receives `get_project_context`, `get_device_info`, `get_device_network` and `get_alarms`; device serial numbers must belong to the current project, and arbitrary API or write calls are unavailable.
