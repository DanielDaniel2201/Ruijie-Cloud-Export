...
2. Log into Ruijie Cloud in Chrome, note the target project's exact name, and copy that tab's URL.
...
The URL identifies the account and region, but not the selected project. Pass the exact project name through `--project` on every command and the Chrome URL through `--url` on the first command. The adapter opens an OpenCLI Adapter tab group, switches the top-left picker to the named project, and verifies the result; later commands on that tab can omit `--url`.
...
- Inner API calls reject absolute URLs, unknown paths, and method mismatches. `--url` selects the account/region; `--project` selects one exact project name.
- There is no generic API, fetch, eval, or configuration command.
- Device SNs must belong to the named project.
